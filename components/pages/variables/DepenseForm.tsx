...
type Props = {
  ...
  createCat: any
  onSubmit: (data: { categorie_id: string; montant: number; date: string; infos: string | null }) => Promise<void>
}

export default function DepenseForm({ open, onOpenChange, categories, espaceId, createCat, onSubmit }: Props) {
      ...
      <DialogContent className="bg-slate-900 border-slate-700">
        ...
        <div className="space-y-4">
          ...
          <Button className="w-full" onClick={async () => {
            if (!txCat) return
            await onSubmit({ categorie_id: txCat, montant: txMontant, date: txDate, infos: txInfos || null })
            setTxMontant(0)
            setTxInfos('')
            onOpenChange(false)
          }}>Ajouter</Button>
