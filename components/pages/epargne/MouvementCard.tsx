...
export default function MouvementCard({ mvt, readOnly, getEnvNom, onEdit, onDelete }: Props) {
  const configs: Record<string, { badge: string; label: string; color: string }> = {
  ...
  const typeConfig = configs[mvt.type] || { badge: 'bg-slate-800 text-slate-400', label: mvt.type, color: 'text-slate-400' }
