...
export default function EpargnePage() {
  ...
  const totalObjectifs = enveloppes
    .filter((e) => e.objectif)
    .reduce((s, e) => s + Number(e.objectif), 0);
  ...
  const onSubmitMvt = async (values: any) => {
    ...
    await createMvt.mutateAsync({
      ...
      type: values.type,
      enveloppe_source_id:
        values.type !== 'alimentation' ? values.source : null,
                ...
                {env.objectif && (
                  <div>
                    <div className="flex justify-between text-xs text-slate-400 mb-1">
                      <span>
                        {pct(Number(env.solde), Number(env.objectif))}%
                      </span>
                      <span>Obj: {formatEuro(Number(env.objectif))}</span>
                    </div>
                    <Progress
                      value={pct(Number(env.solde), Number(env.objectif))}
                      className="h-2"
                    />
                  </div>
                )}
        ...
        <Card className="bg-teal-950 border-teal-800">
          <CardContent className="p-4">
            <div className="flex justify-between">
              <span className="font-semibold">Total LDDS</span>
              <span className="font-bold text-lg text-teal-300">
                {formatEuro(totalSolde)}
              </span>
            </div>
          </CardContent>
        </Card>
